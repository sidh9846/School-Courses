...
class List:
    ...
    def insert(self, i, value):
        ...
        elif i == 0 or i < -(self._count):
            self.prepend(value)

        else:
            current = self._front
            count = 0
            while count != i and current is not None:
                pre = current
                current = current._next
                count += 1

            temp = _List_Node(value, current)
    ...
    def reverse_r(self):
        ...
        self._rear = self._front
        self._front = self.reverse_r_aux(self._front, None)
    ...
    def reverse_r_aux(self, current, previous):
        ...
        if current is not None:
            temp = current._next
            current._next = previous
            previous = self.reverse_r_aux(temp, current)
    ...
    def identical_r_aux(self, node1, node2):
        ...
        if node1 is None:
            ...
            identical = self.identical_r_aux(node1._next, node2._next)
    ...
    def split(self):
        ...
        #
        # target1_current = target1._front
        #
        # while index2 != mid-1:  # -1
        #     target1_current = target1_current._next
        #     index2 += 1
        #
        # target1_current._next = None
        #
        # target1._count = new_length
        # target2._count = new_length
        #
        # self._front = None
        # self._rear = self._front
        # self._count = 0
    ...
    def split_alt_r_aux(self, list1, list2):

        if self._front is not None:
            list1._move_front_to_rear(self)
            self.split_alt_r_aux(list2, list1)

        return

    def _linear_search_r_aux(self, key, node, previous, index):

        if node is None:
            index = -1
        elif node._value != key:
            previous, node, index = self._linear_search_r_aux(
                key, node._next, node, index + 1)
    ...
    def intersection_r_aux(self, source1, source2):
        ...
        if source1 is not None:
            p, c, i = source2._linear_search(source1._value)
            if c is not None:
                p1, c1, i1 = self._linear_search(source1._value)
                if c1 is None:
                    self.append(source1._value)
            self.intersection_r_aux(source1._next, source2)
    ...
    def union_r_aux(self, source):

        if source is not None:
            p, c, i = self._linear_search(source._value)
            if i == -1:
                self.append(source._value)
            self.union_r_aux(source._next)
