...
//public class SingleLink<T> implements Iterable<T> {
	...
	// your code here
    	...
    	// get front node from source
		SingleNode<T> sourceFront = source.front;	
		
		// delete front node in source
		source.front = source.front.getNext();	
		source.length--;
		
		// if source became empty, set all to null
		if (source.length == 0) {
			source.front = null;
			source.rear = source.front;
		}
				
		// if this was empty, set front and rear	else; link new node to rest of list and set only front
		if (this.length == 0) {
